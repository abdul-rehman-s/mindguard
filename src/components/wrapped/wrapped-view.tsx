...
function WoWArrow({ value, suffix = '%' }: { value: number; suffix?: string }) {
  if (value > 0) {
    return (
...
export function WrappedView() {
  const { wrapped, setWrapped } = useAppStore();
  ...
  if (error || !wrapped) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-3">
        ...
        <Button variant="ghost" size="sm" onClick={fetchWrapped}>
          Try again
        </Button>
      </div>
    );
  ...
  const gradeStyle =
    GRADE_STYLES[wrapped.attentionGrade] || GRADE_STYLES.F;
    ...
    >
      ...
      {isEmpty ? (
        <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02] mx-auto max-w-md flex flex-col items-center justify-center py-20 text-center">
          ...
          <Button
            onClick={() => useAppStore.getState().setView('timer')}
        ...
        <>
          ...
          <div className="mb-4 grid gap-4 lg:grid-cols-3">
            ...
            >
              <div className="card-glow glass-card glass-glow-edge border-white/[0.06] relative h-full overflow-hidden bg-gradient-to-br from-emerald-500/[0.06] via-white/[0.02] to-transparent">
                <div className="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-emerald-500/[0.08] blur-3xl" />
                <div className="relative p-6 sm:p-8">
                  ...
                  <div className="flex items-baseline gap-2">
                    <AnimatedNumber
                      value={wrapped.totalFocusHours}
                      decimals={1}
                      className="bg-gradient-to-br from-zinc-50 to-emerald-200 bg-clip-text text-5xl font-bold tabular-nums text-transparent sm:text-6xl"
                    />
                    <span className="text-xl font-medium text-zinc-500">hrs</span>
                  </div>
                  <p className="mt-2 text-xs text-zinc-500">
                    {wrapped.sessionCount} session{wrapped.sessionCount === 1 ? '' : 's'} this week
                  </p>
                  <div className="mt-4">
                    <WoWArrow value={wrapped.weekOverWeek.focusChange} />
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div
                className={cn(
                  'card-glow glass-card glass-glow-edge border-white/[0.06] relative h-full overflow-hidden bg-gradient-to-br',
                  gradeStyle.bg
                )}
              >
                <div className="relative flex flex-col items-center justify-center p-6 sm:p-8">
                  <div className="mb-2 flex items-center gap-2">
                    <Award className={cn('h-3.5 w-3.5', gradeStyle.color)} />
                  ...
                  <motion.div
                    ...
                    transition={{ delay: 0.4, duration: 0.6, ease: EASE, type: 'spring' }}
                    className={cn(
                      'flex h-24 w-24 items-center justify-center rounded-2xl ring-2',
                      gradeStyle.ring
                    )}
                  >
                    <span className={cn('text-5xl font-bold', gradeStyle.color)}>
                      {wrapped.attentionGrade}
                    </span>
                  </motion.div>
                  <p className={cn('mt-3 text-xs font-medium', gradeStyle.color)}>
                    {gradeStyle.label}
                  </p>
                  {wrapped.attentionScore !== undefined && (
                    <p className="mt-0.5 text-[10px] tabular-nums text-zinc-600">
                      {wrapped.attentionScore}/100 composite score
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <WrappedStatCard
              ...
              value={
                wrapped.deepestSession
                  ? `${wrapped.deepestSession.duration}m`
                  : '—'
              }
              subtitle={wrapped.deepestSession?.mission || 'No session this week'}
              accent="purple"
            />
            ...
            <WrappedStatCard
              ...
              value={
                wrapped.bestDay
                  ? `${wrapped.bestDay.minutes}m`
                  : '—'
              }
              subtitle={
                wrapped.bestDay
                  ? `${wrapped.bestDay.day} · ${wrapped.bestDay.sessions} session${wrapped.bestDay.sessions === 1 ? '' : 's'}`
                  : 'No data'
              }
              accent="emerald"
            />
            ...
            <WrappedStatCard
              ...
              icon={Clock}
              label="Peak Hour"
              value={wrapped.mostProductiveHour?.hour || '—'}
              subtitle={
                wrapped.mostProductiveHour
                  ? `${wrapped.mostProductiveHour.sessions} session${wrapped.mostProductiveHour.sessions === 1 ? '' : 's'} · avg ${wrapped.mostProductiveHour.avgMinutes}m`
                  : 'No sessions logged'
              }
              accent="amber"
            />
            ...
            <WrappedStatCard
              ...
              value={`${wrapped.longestStreak}d`}
              subtitle={
                wrapped.weekOverWeek.streakChange === 0
                  ? 'Same as last week'
                  : `${wrapped.weekOverWeek.streakChange > 0 ? '+' : ''}${wrapped.weekOverWeek.streakChange} vs last week`
              }
              accent="emerald"
            />
            ...
            <WrappedProgressCard
              ...
              value={wrapped.missionCompletionRate}
              suffix="%"
              subtitle={
                wrapped.missionsCompleted !== undefined && wrapped.missionsCreated !== undefined
                  ? `${wrapped.missionsCompleted}/${wrapped.missionsCreated} missions completed`
                  : undefined
              }
              accent="purple"
              wowValue={wrapped.weekOverWeek.missionRateChange}
              wowSuffix="pp"
            />
            ...
            <WrappedProgressCard
              ...
              value={wrapped.reflectionRate}
              suffix="%"
              subtitle={
                wrapped.reflectionsWritten !== undefined && wrapped.reflectionDaysPossible
                  ? `${wrapped.reflectionsWritten}/${wrapped.reflectionDaysPossible} days`
                  : undefined
              }
              accent="sky"
            />
          ...
          >
            <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02]">
              ...
              <div className="grid grid-cols-3 gap-2 px-5 pb-5">
                <WoWCard
                  label="Focus Time"
                  change={wrapped.weekOverWeek.focusChange}
                  thisValue={`${wrapped.totalFocusHours}h`}
                  lastValue={wrapped.lastWeek ? `${wrapped.lastWeek.totalFocusHours}h` : '—'}
                />
                <WoWCard
                  label="Sessions"
                  change={wrapped.weekOverWeek.sessionChange}
                  thisValue={wrapped.sessionCount.toString()}
                  lastValue={
                    wrapped.lastWeek ? wrapped.lastWeek.sessionCount.toString() : '—'
                  }
                />
                <WoWCard
                  label="Streak"
                  change={wrapped.weekOverWeek.streakChange}
                  thisValue={`${wrapped.longestStreak}d`}
                  lastValue={
                    wrapped.lastWeek ? `${wrapped.lastWeek.longestStreak}d` : '—'
                  }
                />
              </div>
            </div>
          ...
          >
            <p className="text-[11px] text-zinc-600">
              Generated from {wrapped.sessionCount} real focus session{wrapped.sessionCount === 1 ? '' : 's'} this week.
            </p>
          </motion.div>
        </>
      )}
...
function WrappedStatCard({
  ...
  subtitle?: string;
  delay: number;
  accent: 'emerald' | 'amber' | 'purple' | 'sky';
}) {
  const accentMap = {
    emerald: 'text-emerald-400 bg-emerald-500/[0.08]',
    amber: 'text-amber-400 bg-amber-500/[0.08]',
    purple: 'text-purple-400 bg-purple-500/[0.08]',
    sky: 'text-sky-400 bg-sky-500/[0.08]',
  };
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
...
function WrappedProgressCard({
  ...
  const accentConfig = {
    emerald: {
      iconBg: 'bg-emerald-500/[0.08] text-emerald-400',
      bar: 'from-emerald-500/80 to-teal-400/60',
    },
    amber: {
      iconBg: 'bg-amber-500/[0.08] text-amber-400',
      bar: 'from-amber-500/80 to-orange-400/60',
    },
    purple: {
      iconBg: 'bg-purple-500/[0.08] text-purple-400',
      bar: 'from-purple-500/80 to-fuchsia-400/60',
    },
    sky: {
      iconBg: 'bg-sky-500/[0.08] text-sky-400',
      bar: 'from-sky-500/80 to-cyan-400/60',
    },
  };
      ...
      <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02] h-full">
        <div className="p-5">
          ...
          <div className="mt-1 flex items-baseline gap-1">
            <AnimatedNumber
              value={value}
              className="text-2xl font-bold tabular-nums text-zinc-50"
            />
          ...
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-white/[0.04]">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${value}%` }}
              transition={{ delay: delay + 0.2, duration: 0.8, ease: EASE }}
    ...
    <div className="rounded-lg border border-white/[0.04] bg-white/[0.02] p-3">
      ...
      <div className="flex items-baseline justify-between gap-2">
        <div>
          ...
          <p className="text-sm font-semibold tabular-nums text-zinc-100">{thisValue}</p>
        </div>
