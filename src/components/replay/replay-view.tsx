    ...
    >
      ...
      <div className="mb-8 flex flex-col gap-4 pt-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2">
            ...
            <span className="text-xs font-medium text-zinc-200">{prettyDate(date)}</span>
      ...
      {loading ? (
        ...
        <div className="flex h-64 flex-col items-center justify-center gap-3">
          ...
          <Button variant="ghost" size="sm" onClick={() => fetchReplay(date)}>
            Try again
          </Button>
        </div>
        ...
        <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02] flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mb-1 text-sm font-medium text-zinc-400">No events on {prettyDate(date)}</p>
              ...
              <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02] max-h-[640px] overflow-y-auto">
                ...
                <div className="px-5 pb-5">
                  <div className="relative">
                    {/* Timeline line */}
                    <div className="absolute left-[15px] top-2 bottom-2 w-px bg-gradient-to-b from-emerald-500/40 via-white/[0.08] to-transparent" />
              ...
              <div className="card-glow glass-card glass-glow-edge border-white/[0.06] bg-white/[0.02]">
                ...
                <div className="grid grid-cols-2 gap-2 px-5 pb-5">
                  <SummaryStat
                    icon={Clock}
                    label="Focus time"
                    value={`${data.summary.totalMinutes}m`}
                  />
                  <SummaryStat
                    icon={Timer}
                    label="Sessions"
                  ...
                  <SummaryStat
                    icon={Trophy}
                    label="Missions done"
                    value={data.summary.missionsCompleted}
                  />
                  <SummaryStat
                    icon={BookOpen}
                    label="Reflection"
                    value={data.summary.reflectionWritten ? 'Yes' : '—'}
                  />
                  <SummaryStat
                    icon={Timer}
                    label="Longest"
                    value={`${data.summary.longestSession}m`}
                  />
                  <SummaryStat
                    icon={Clock}
                    label="Best hour"
                    value={data.summary.bestHour ?? '—'}
                  />
                </div>
              </div>
            </div>
